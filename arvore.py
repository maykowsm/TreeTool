import Draft, FreeCAD, math, random, Part,Arch, FreeCADGui, Mesh


interacao = 2				#Número de vezes que o algoritimo irá rodar (aumenta o numero de galhos)

diametro_tronco = 600		#Diâmetro inicial do tronco da vegetação
taxa_reducao_tronco = 0.7 #taxa na qual os galhos reduzem seu diametro

tamanho_tronco = 2000		#Altura do tronco inicial (influencia na altura e numero de galhos)
inclinação_max_tronco = 200	#desvio máximo do tronco inicial em relação a vertical
...
tipo_folhas = 2				#1 - folhas esfericas lisas; 2 - folhas esfericas multifacetadas





def folhasEsfericas(pos):
	esfera = App.ActiveDocument.addObject("Part::Sphere","folha")
	esfera.Radius = random.randint(tam_min_folhas, tam_max_folhas)
	esfera.Placement.Base = FreeCAD.Vector(pos)
	esfera.ViewObject.DisplayMode = 'Shaded'
	FreeCAD.ActiveDocument.recompute()

def folhasFacetadas(pos):
	#cria a malha
	malha = App.ActiveDocument.addObject("Mesh::Sphere","malha")
	malha.Radius= random.randint(tam_min_folhas, tam_max_folhas)
	malha.Sampling= 5
	malha.Placement.Base = FreeCAD.Vector(pos)
	FreeCAD.ActiveDocument.recompute()
	
	#criando o solido das folhas
	esfera = FreeCAD.ActiveDocument.addObject('Part::Feature', 'folha')
	forma = Part.Shape()
	forma.makeShapeFromMesh(malha.Mesh.Topology, 0.1)
	esfera.Shape = forma
	esfera.purgeTouched()
	FreeCAD.ActiveDocument.recompute()


	#Apaga a malha criada
	FreeCAD.ActiveDocument.removeObject(malha.Name)
	del(forma)
	FreeCAD.ActiveDocument.recompute()
...
def ramo(comprimento, angulo, point, diametro):
	...
	if comprimento > tam_max_ramos :
		...
		if tipo_folhas == 1:
			folhasEsfericas(point)
		elif tipo_folhas == 2:
			folhasFacetadas(point)

#Cria os pontos para o tronco
...
lista_linhas = list(filter(lambda obj: 'rascunho' in obj.Label, lista_objetos))
...
FreeCAD.ActiveDocument.recompute()
compound_ramos.ViewObject.LineWidth = 1
compound_ramos.ViewObject.PointSize = 1